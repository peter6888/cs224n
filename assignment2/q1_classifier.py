...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("transformation"):
            self.W = tf.get_variable("W", shape=(self.config.n_features, self.config.n_classes), dtype=tf.float32, initializer=tf.zeros_initializer)
            self.b = tf.get_variable("b", shape=(self.config.n_classes), dtype=tf.float32, initializer=tf.zeros_initializer)
            self.yhat = softmax(tf.add(tf.matmul(self.input_placeholder, self.W, name="yhat"), self.b))
        pred = self.yhat
